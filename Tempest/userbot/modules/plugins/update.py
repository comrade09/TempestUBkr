...
async def gen_chlog():
    ...
    recent_updates = requests.get("https://api.github.com/repos/TempestNetwork/TempestUB/events").json()
...
async def update_handler(_, m):
    try:
        ...
        try:
            ...
            repo = Repo.init()
            origin = repo.create_remote("upstream", TEMPEST_REPO)
            origin.fetch()
        ...
        try:
            repo.create_remote("upstream", TEMPEST_REPO)
