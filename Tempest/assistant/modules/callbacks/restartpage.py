...
async def _confirm_restart(_, cb: CallbackQuery):
    try:
        ...
        await cb.edit_message_text(
            text=app.restart_tab_string("`Trying to restart Tempest userbot . . .`"),
        ...
        if not app.heroku_app():
            ...
            text = "`Please wait 2-3 minutes to restart Tempest userbot . . .`"
            final_text = text if res else "`Failed to restart Tempest userbot, do it manually . . .`"
            await cb.edit_message_text(
