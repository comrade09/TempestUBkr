class Strings(object):
    def about_tab_string(self):
        ...
        text += "I am Shion made by Zephys.\n"
        ...
        text += "which are related to Tempest userbot then just ask in\n"
        text += f"[support group](https://t.me/TempestSupports)"

        return text


    def close_tab_string(self):
        text = "Welcome to Tempest UserBot.\n"
        text += "This is your Helpdex, Tap on open button to get more buttons,\n"
        text += "which will help you to understand  operate your userbot & assistant ( Shion )\n"
        text += "\n\n• Menu is closed"
    ...
    def ialive_tab_string(self):
        ...
        text += f"**⟜ TempestUB:** `{self.userbot_version}`\n"
        text += f"**⟜ Python:** `{self.python_version}`\n"
        text += f"**⟜ Pyrogram:** `{self.pyrogram_version}`\n"
        text += f"**⟜ uptime:** `{self.uptime()}`\n"
